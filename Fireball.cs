...
function EOTWFireballProjectile::onCollision(%this, %obj, %col, %fade, %pos, %normal)
{
	...
	while(isObject(%hit = containerSearchNext())) if(%hit.canBeFireballed)
	...
	{
		if(!isObject(containerRaycast(%loc = %hit.getHackPosition(), %pos, $Typemasks::fxBrickObjectType)))
		{
			%power = 10 - mFloor(vectorDist(%pos, %hit.getHackPosition()));
			if(%power > 0)
			{
				%hit.addVelocity(vectorScale(vectorNormalize(vectorSub(%loc, %pos)), %power * 5));
				%hit.burningTime += %power;
				if(!isEventPending(%hit.burnLoop))
					%hit.burnLoop(getSimTime());
			}
		}
	}
}